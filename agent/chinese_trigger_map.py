"""
中文触发词 → GitHub Skills 子领域映射表。

问题：GitHub Skills 的 YAML frontmatter 只包含英文 tags，
中国用户输入中文时关键词匹配效果差。

方案：构建轻量级中文触发词 → 子领域映射表，
搜索时先用中文输入匹配候选子领域，再在候选子领域内用英文 tags 精确检索。
"""

# 每个子领域对应一组中文触发词（含英文缩写常用术语）
